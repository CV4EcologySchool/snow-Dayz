...
from dataset import SnowDepthSequenceDataset
...
if config.split == 'traditional':
    total = len(image_paths) * 0.01 ## we just want 10% of data for testing
    ...
    # ]
    wa_cams_val = ['TWISP-U-01']
    # 'TWISP-R-01', 'CUB-H-02', 'CUB-L-02', 'CUB-M-02','CEDAR-H-01',
...
# Create dataset
train_dataset = SnowDepthSequenceDataset(train_paths, metadata, transform=transform, sequence_length=config.sequence_length)
val_dataset = SnowDepthSequenceDataset(val_paths, metadata, transform=transform, sequence_length=config.sequence_length)
test_dataset = SnowDepthSequenceDataset(test_paths, metadata, transform=transform, sequence_length=config.sequence_length)
...
def fit(model, dataloader):
    ...
    for i, data in tqdm.tqdm(enumerate(dataloader)):
        ...
        images, labels, filenames = data['images'], data['label'], data['filenames']
